...
const basket = {
		...
		svc.cartList(function(result){
			...
			result.forEach(cart =>{
				...
				temp.find('div.basketprice').prepend($('<input />')
							.attr({'type':'hidden', 'name': 'p_name', 'id':'p_price'+cart.no})
							.addClass('p_price')
							.val(cart.price)
				);
	...
	changePNum(no){
		...
		console.log(event);
		let qty = -1;
		if(event.target.nodeNmae == "I"){
			if(event.target.className.indexOf('up') != -1){
				qty = 1;
			}
		}else if(event.target.nodeNmae == "INPUT"){
		...
		(result) =>{
			...
			if(result.retCode == 'Success'){
				qtyElem.val(partseInt(curQty) + qty);
				sumElem.text((parseInt(curQty)+qty) * parseInt(price));
